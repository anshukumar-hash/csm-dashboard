...
function Parse-Day($s) {
    ...
    try {
        $clean = $s.Trim()
        # e.g. "23 May, 2026"
        return [datetime]::ParseExact($clean, 'd MMM, yyyy', [System.Globalization.CultureInfo]::InvariantCulture)
    } catch {
        try { return [datetime]$s } catch { return [datetime]::MinValue }
    }
}

# RAG ranking: Red > Amber > Yellow > Green > NA/blank
function RAG-Rank($s) {
...
function Worst-RAG($values) {
    ...
    foreach ($v in $values) {
        $r = RAG-Rank $v
        if ($r -gt $rank) { $rank = $r; $best = $v }
    }
    return $best
}

# Payment status ranking: overdue > no invoice raised > paid > blank
function Pay-Rank($s) {
    if ([string]::IsNullOrWhiteSpace($s)) { return 0 }
    switch ($s.Trim().ToLower()) {
        'overdue'             { return 3 }
...
# ----- load -----
$viniA   = Read-Tsv "$dir\vini_a.tsv"
$viniB   = Read-Tsv "$dir\vini_b.tsv"
$payRows = Read-Tsv "$dir\payment.tsv"
Write-Output ("Loaded vini_a=" + $viniA.Count + " vini_b=" + $viniB.Count + " payment=" + $payRows.Count)

# Combine vini sources + DEDUPE on (EnterpriseID|RooftopID|AgentType|Day)
# Reason: gid=1616842841 was exported in two markdown chunks; ~85% of rows overlap.
$viniAll = New-Object System.Collections.Generic.List[hashtable]
$seen = New-Object System.Collections.Generic.HashSet[string]
foreach ($src in @($viniA, $viniB)) {
    foreach ($r in $src) {
        $key = ($r['Enterprise ID'] + '|' + $r['Rooftop ID'] + '|' + $r['Agent Type'] + '|' + $r['Day'])
        if ($seen.Add($key)) { $viniAll.Add($r) }
    }
}
Write-Output ("Combined+deduped vini rows: " + $viniAll.Count + " (from " + ($viniA.Count + $viniB.Count) + " raw)")

# ----- Roll Vini up to one row per (Enterprise ID) using latest available day -----
# First, find latest day per Enterprise ID
$latestByEnt = @{}
...
Write-Output ("Unique enterprises in vini: " + $latestByEnt.Count)

# Aggregate Vini metrics at enterprise level on its latest day
$viniRollup = @{}
foreach ($r in $viniAll) {
    $entId = $r['Enterprise ID']
    if ([string]::IsNullOrWhiteSpace($entId)) { continue }
    ...
    if (-not $viniRollup.ContainsKey($entId)) {
        $viniRollup[$entId] = @{
            'Enterprise ID'       = $entId
            'Enterprise Name'     = $r['Enterrpise Name']
            'CSM Name'            = $r['CSM Name']
            'Customer Type'       = $r['Customer Type']
            'Customer Subtype'    = $r['Customer Subtype']
            'Customer Segment'    = $r['Customer Segment']
            'Region'              = $r['Region']
            'Stage'               = $r['Stage']
            'Latest Day'          = $r['Day']
            ...
            'Usage_Touched'       = 0.0
            'Usage_Qualified'     = 0.0
            'Usage_ApptBooked'    = 0.0
            'Usage_ConvRate_sum'  = 0.0
            'Usage_ConvRate_cnt'  = 0
            'RoI_ApptValue'       = 0.0
            'RoI_Factor_sum'      = 0.0
            'RoI_Factor_cnt'      = 0
    ...
    $rf = $r['RoI?/RoI (Factor)']
    if ($rf -eq 'Infinity') {
        # treat infinity as worst-positive signal - skip from average
    } else {
        $rfn = To-Number $rf
...
Write-Output ("Vini rollup enterprises: " + $viniRollup.Count)

# ----- Roll Payment up to enterprise level -----
$payRollup = @{}
...
# ----- Full-outer join on Enterprise ID -----
$allEntIds = New-Object System.Collections.Generic.HashSet[string]
foreach ($k in $viniRollup.Keys) { [void]$allEntIds.Add($k) }
foreach ($k in $payRollup.Keys)  { [void]$allEntIds.Add($k) }
Write-Output ("Union enterprises: " + $allEntIds.Count)
...
foreach ($entId in $allEntIds) {
    ...
    # Source flag
    if ($v -ne $null -and $p -ne $null) { $row['Source'] = 'Vini + Payment' }
    elseif ($v -ne $null) { $row['Source'] = 'Vini only' }
    else { $row['Source'] = 'Payment only' }
...
foreach ($r in $outRows[1..($outRows.Count-1)]) {
    ...
    if ($rowMap['Source'] -eq 'Vini only')   { $noPay++ }
    if ($rowMap['Source'] -eq 'Payment only') { $noVini++ }
}
Write-Output ""
Write-Output "=== QA ==="
Write-Output ("Unassigned CSM rows: " + $unassigned)
Write-Output ("Enterprises in Vini but no Payment match: " + $noPay)
Write-Output ("Enterprises in Payment but no Vini match: " + $noVini)
